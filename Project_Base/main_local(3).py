...
def on_mqtt_message(client, userdata, msg):
    ...
    try:
        ...
        db = SessionLocal()
        tele = database_models.SensorData(
...
def db_monitor_thread():
    ...
    while True:
        try:
            db = SessionLocal()
            open_session = db.query(database_models.SystemSession).filter(
                database_models.SystemSession.end_time == None
            ).first()
...
def start_session_in_db(operator_id):
    ...
    try:
        ...
        new_s = database_models.SystemSession(
...
def stop_session_in_db():
    ...
    try:
        db = SessionLocal()
        session = db.query(database_models.SystemSession).filter(
            database_models.SystemSession.id == active_session_id
        ).first()
...
class FeatureExtractor:
    def __init__(self):
        ...
        self.model.layer2.register_forward_hook(
            lambda m, i, o: self.features.update({'layer2': o})
        )
        self.model.layer3.register_forward_hook(
            lambda m, i, o: self.features.update({'layer3': o})
        )
        self.model.eval()
...
print(f"✅ PatchCore loaded! Threshold: {threshold:.4f}")

with open(FEWSHOT_PATH, 'rb') as f:
    fs_data        = pickle.load(f)
class_features = fs_data['class_features']
class_names    = fs_data['class_names']
print(f"✅ Few-shot loaded! Classes: {class_names}")




def predict_anomaly(features):
    distances  = pairwise_distances(features.reshape(1, -1), memory_bank)
    score      = distances.min()
    is_anomaly = score > threshold
    return is_anomaly, score

def predict_defect_type(features):
    min_distance    = float('inf')
    predicted_class = None
    for class_name, refs in class_features.items():
        distances = pairwise_distances(features.reshape(1, -1), refs)
...
def predict(image):
    ...
    if not is_anomaly:
        return "Good", score, (0, 255, 0)
    else:
        defect_type = predict_defect_type(features)
        return defect_type, score, (0, 0, 255)
        ...
        try:
            ...
            if (mw10_value == 1) or (take_photo is not None):
                if active_session_id is None:
                    ...
                    result, score, color = predict(pil_image)
                    confidence = min(100,(score/threshold)*100)
                    if result == "Good":
                        status = "Accepted"
                    else:
                        status = "Defected"
                    print(f"🤖 AI Result: {result} | score: {score:.2f}% | conf: {confidence} ")
                    ...
                    if confidence < 100:
                        ...
                        try:
                            ...
                            db = SessionLocal()
                            new_insp = database_models.Inspection(
                                session_id=active_session_id,
                                status=status,
                                defect_category=result,
